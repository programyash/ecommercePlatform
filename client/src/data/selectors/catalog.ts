...
export function productPrice(product: Product): number {
  ...
  const prices = list.map((v) => v.price).filter((p) => Number.isFinite(p) && p > 0)
  return prices.length > 0 ? Math.min(...prices) : (list[0]?.price ?? 0)
}

export function productMrp(product: Product): number {
  const list = activeVariants(product)
  if (list.length === 0) return 0
  const mrps = list.map((v) => v.mrp).filter((m) => Number.isFinite(m) && m > 0)
  return mrps.length > 0 ? Math.min(...mrps) : (list[0]?.mrp ?? productPrice(product))
}

export function productDiscount(product: Product): number {
  const mrp = productMrp(product)
  const price = productPrice(product)
  return mrp > 0 ? discountPercent(mrp, price) : 0
}

export function productStock(product: Product): number {
  const list = activeVariants(product)
  if (list.length === 0) return 0
  return list.reduce((sum, variant) => sum + (Number.isFinite(variant.stock) ? Math.max(0, variant.stock) : 0), 0)
}

export function productStockStatus(product: Product): StockStatus {
  const total = productStock(product)
  const list = activeVariants(product)
  const threshold = list.length > 0 ? Math.max(...list.map((variant) => variant.lowStockAt ?? 5)) : 5
...
/** Shoppers see live listings from active (and onboarding) sellers. */
export function isShoppable(view: View, product: Product): boolean {
  ...
  return !seller || seller.status === 'active' || seller.status === 'under_review'
}
...
export function getProductDetail(view: View, productId: ID): ProductDetail | null {
  const product = view.productById.get(productId) ?? view.productBySlug.get(productId)
  if (!product) return null
  const seller = view.sellerById.get(product.sellerId)
