...
export function imageUrl(ref: MediaRef, { w, h, q = 75 }: ImageUrlOptions): string {
  const key = mediaImageId(ref)
  if (typeof key === 'string' && (key.startsWith('http://') || key.startsWith('https://') || key.startsWith('blob:') || key.startsWith('data:') || key.startsWith('/'))) {
    return key
  }
  const asset = IMAGES[key]
  if (!asset) {
    return `${BASE}1523275335684-37898b6baf30?w=${w}&q=${q}&auto=format&fit=crop`
  }
...
export function imageSrcSet(ref: MediaRef, ratio: number | null, widths: readonly number[] = [200, 320, 480, 640, 960, 1280]): string {
  const key = mediaImageId(ref)
  if (typeof key === 'string' && (key.startsWith('blob:') || key.startsWith('data:'))) {
    return key
  }
  return widths
