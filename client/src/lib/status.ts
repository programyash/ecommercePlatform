...
export const STATUS_META: MetaMap = {
  shipment: {
    ...
    processing: { label: 'Processing', tone: 'info', icon: 'clock', description: 'Seller is preparing the order.' },
    packed: { label: 'Packed', tone: 'info', icon: 'package', description: 'Packed and waiting for the courier.' },
    ready_for_pickup: { label: 'Ready for pickup', tone: 'info', icon: 'truck', description: 'Waiting for pickup.' },
    picked_up: { label: 'Picked up', tone: 'info', icon: 'truck', description: 'Picked up by the courier.' },
    in_transit: { label: 'In transit', tone: 'info', icon: 'truck', description: 'In transit to the destination.' },
...
export function statusMeta<D extends StatusDomain>(domain: D, status: StatusDomains[D] | string): StatusMeta {
  ...
  return map[key as string] ?? FALLBACK_META
}
...
export const SHIPMENT_TRANSITIONS: Record<ShipmentStatus, Transition<ShipmentStatus>[]> = {
  ...
  confirmed: [
    ...
    { to: 'packed', actors: ['seller', 'admin'] }, // fallback
    { to: 'cancelled', actors: ['customer', 'seller', 'admin'] },
  ],
  processing: [
  ...
  packed: [
    { to: 'ready_for_pickup', actors: ['seller', 'admin'] },
    { to: 'shipped', actors: ['seller', 'admin'] }, // fallback
    { to: 'cancelled', actors: ['seller', 'admin'] },
  ],
  ready_for_pickup: [
    { to: 'picked_up', actors: ['courier', 'system', 'admin'] },
  ],
  picked_up: [
    { to: 'in_transit', actors: ['courier', 'system', 'admin'] },
  ],
  in_transit: [
    { to: 'out_for_delivery', actors: ['courier', 'system', 'admin'] },
  ],
...
export const SHIPMENT_PROGRESS: readonly ShipmentStatus[] = [
  'placed', 'confirmed', 'processing', 'packed', 'ready_for_pickup', 'picked_up', 'in_transit', 'shipped', 'out_for_delivery', 'delivered',
]
...
export function isOpenShipmentStatus(status: ShipmentStatus): boolean {
  return status === 'placed' || status === 'confirmed' || status === 'processing' || status === 'packed'
}
...
export const SELLER_ORDER_TABS: readonly SellerOrderTab[] = [
  ...
  { id: 'to_pack', label: 'To pack', statuses: ['confirmed', 'processing'], emptyLine: 'Nothing to pack right now.' },
  { id: 'ready_to_ship', label: 'Ready to ship', statuses: ['packed', 'ready_for_pickup'], emptyLine: 'No packed orders waiting for the courier.' },
  { id: 'in_transit', label: 'In transit', statuses: ['picked_up', 'in_transit', 'shipped', 'out_for_delivery'], emptyLine: 'Nothing is on its way at the moment.' },
