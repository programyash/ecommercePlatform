        ...
        <>
          ...
          <div className="grid gap-4 lg:grid-cols-3">
            <ChartCard title="Shipments by status" description="Click a bar to open that queue." loading={loading}>
              <BarBreakdownChart
                data={(data?.byStatus ?? []).map((row: any) => ({
                ...
                cellColors={(data?.byStatus ?? []).map((row: any) => STATUS_TONE_COLOR[statusMeta('shipment', row.status).tone])}
            ...
            >
              <DonutChart
                data={(data?.payments ?? []).map((row: any) => ({
                  label: (PAYMENT_METHOD_META as any)[row.method]?.label ?? row.method,
                  value: row.orders,
                }))}
                centerValue={formatNumber((data?.payments ?? []).reduce((sum: number, row: any) => sum + row.orders, 0))}
            ...
            <ChartCard title="Category share" description="GMV by top-level category." loading={loading}>
              <DonutChart
                data={(data?.categories ?? []).slice(0, 6).map((row: any) => ({ label: row.name, value: row.gmv }))}
                centerValue={formatINRCompact((data?.categories ?? []).reduce((sum: number, row: any) => sum + row.gmv, 0))}
                centerLabel="GMV"
