...
export default function AdminProductDetailPage() {
  ...
  const approve = async () => {
    ...
    try {
      const token = getAuthToken('admin') || localStorage.getItem('token') || sessionStorage.getItem('token')
      if (token) {
        await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5000/api/v1'}/admin/products/${product.id}/moderation`, {
          method: 'PUT',
          headers: {
