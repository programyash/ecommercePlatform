...
export default function AdminPayoutsPage() {
  ...
  const open = rows.find((row: any) => row.payout.id === statement)
  ...
  const payMany = async (ids: string[], clear: () => void) => {
    const targets = rows.filter((row: any) => ids.includes(row.payout.id) && row.payout.status !== 'paid' && row.payout.status !== 'on_hold')
    if (!targets.length) {
      toast.message('Nothing to pay', { description: 'The batches you selected are already paid, or on hold.' })
      return
    }
    const total = targets.reduce((sum: number, row: any) => sum + row.payout.net, 0)
    const ok = await confirm({
    ...
    if (!ok) return
    targets.forEach((row: any) => dbActions.markPayoutPaid(row.payout.id))
    clear()
          ...
          query.status === 'success' ? (
            <>
              ...
              <span className="tabular">
                {formatINR(
                  rows.filter((row: any) => row.payout.status !== 'paid').reduce((sum: number, row: any) => sum + row.payout.net, 0),
                )}{' '}
                still to pay
              </span>
            </>
          ...
          bulkActions={(ids, clear) => (
            <>
              ...
              <ExportButton
                ...
                headers={csvHeaders}
                rows={() => rows.filter((row: any) => ids.includes(row.payout.id)).map(csvRow)}
              />
            </>
          )}
          toolbar={
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="overflow-x-auto rounded-card border border-border">
                <table className="w-full border-collapse text-left">
                  ...
                  <tbody>
                    {open.payout.lines.map((line: any) => (
