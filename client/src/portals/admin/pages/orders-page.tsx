...
export default function AdminOrdersPage() {
  ...
  const query = useApiQuery<any[]>('/admin/orders', [useSocketStore(s => s.orderTick)])

  const allRows = (query.data ?? []).map((order: any) => {
    const sellersMap = new Map();
    let units = 0;
    
    const normalizedShipments = (order.shipments || []).map((shipment: any) => ({
      ...shipment,
      status: String(shipment.status || 'placed').toLowerCase(),
    }));

    for (const shipment of normalizedShipments) {
      if (shipment.seller) {
         sellersMap.set(shipment.seller.id, shipment.seller);
      }
    ...
    const rawMethod = String(order.paymentMethod || 'cod').toLowerCase();
    const rawStatus = String(order.paymentStatus || 'pending').toLowerCase();
    
    return {
      order: {
        ...order,
        totals: order.totals || { total: order.paymentAmount || 0 },
        payment: {
          method: rawMethod,
          status: rawStatus,
          amount: order.paymentAmount,
          txnRef: order.paymentTxnRef,
          paidAt: order.paymentPaidAt,
          failureReason: order.paymentFailed
        }
      },
  ...
  // Apply filtering
  if (view !== 'all') {
    if (view === 'attention') rows = rows.filter(r => r.shipments.some((s: any) => ['placed', 'confirmed', 'packed', 'processing'].includes(s.status)));
    if (view === 'cod') rows = rows.filter(r => r.order.payment.method === 'cod');
    if (view === 'returns') rows = rows.filter(r => r.shipments.some((s: any) => s.returnId || (s.returnReqs && s.returnReqs.length > 0)));
    if (view === 'cancelled') rows = rows.filter(r => r.shipments.some((s: any) => s.status === 'cancelled'));
  }
  if (payment) rows = rows.filter(r => r.order.payment.status === payment.toLowerCase());
  if (method) rows = rows.filter(r => r.order.payment.method === method.toLowerCase());
  if (status) rows = rows.filter(r => r.shipments.some((s: any) => s.status === status.toLowerCase()));
  if (sellerId) rows = rows.filter(r => r.sellers.some((s: any) => s.id === sellerId));
  if (state) rows = rows.filter(r => r.order.shipTo?.state === state);
  if (amount) {
     if (amount.min !== undefined) rows = rows.filter(r => (r.order.totals?.total ?? 0) >= amount.min!);
     if (amount.max !== undefined) rows = rows.filter(r => (r.order.totals?.total ?? 0) <= amount.max!);
  }
  if (q) {
     const queryStr = q.toLowerCase();
     rows = rows.filter(r => 
        r.order.id.toLowerCase().includes(queryStr) || 
        (r.customer?.name || '').toLowerCase().includes(queryStr) ||
        r.shipments.some((s: any) => s.items?.some((i: any) => i.title?.toLowerCase().includes(queryStr)))
     );
  }
  ...
  const csvRow = (row: Row) => [
    row.order.id,
    row.order.placedAt,
    row.customer?.name ?? row.order.shipTo?.name ?? 'Customer',
    row.order.shipTo?.city ?? '',
    row.order.shipTo?.state ?? '',
    row.sellers.map((seller) => seller.displayName).join(' | '),
    row.units,
    row.order.totals.total,
    PAYMENT_METHOD_META[row.order.payment.method as PaymentMethod]?.short ?? row.order.payment.method ?? 'COD',
  ...
  const columns: Column<Row>[] = [
    ...
    },
    {
      ...
      header: 'Shopper',
      mobile: 'subtitle',
      sortValue: (row) => row.customer?.name ?? row.order.shipTo?.name ?? 'Customer',
      cell: (row) => (
        <span className="flex min-w-0 flex-col">
          <span className="truncate">{row.customer?.name ?? row.order.shipTo?.name ?? 'Customer'}</span>
    ...
    },
    {
      ...
      cell: (row) => (
        <span className="flex flex-col items-start gap-1">
          <span className="whitespace-nowrap">{PAYMENT_METHOD_META[row.order.payment.method as PaymentMethod]?.short ?? row.order.payment.method ?? 'COD'}</span>
          ...
          <details className="group min-w-0">
            <summary className="flex cursor-pointer list-none items-center gap-2 rounded-badge py-0.5 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring [&::-webkit-details-marker]:hidden">
              ...
              <span className="flex shrink-0 items-center gap-1">
                {row.shipments.map((shipment: any) => (
                ...
                return (
                  <li key={shipment.id} className="flex flex-col gap-1">
                    ...
                    {shipment.items?.length > 0 && (
                      <span className="type-caption text-fg-subtle truncate">
                        ↳ {shipment.items.map((i: any) => i.title).join(', ')}
                      </span>
                    )}
                  </li>
                )
