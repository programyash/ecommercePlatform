import { Ban, Check, Copy, Ellipsis, ExternalLink, NotebookPen, Package, Truck, Undo2, User } from 'lucide-react'
...
import { PAYMENT_METHOD_META, SHIPMENT_PROGRESS, deriveOrderSummary, statusMeta, statusOptions } from '@/lib/status'
import { gstAmount } from '@/lib/tax'
import { AuditLog } from '../components/audit-log'
import { LoadFailed, RecordNotFound } from '../components/record-states'
import { getAuthToken, ensureAuthToken } from '@/lib/auth-token'
...
export default function AdminOrderDetailPage() {
  ...
  const query = useApiQuery<any>(`/admin/orders/${orderId}`, [orderId, orderTick])

  // Transform data
  if (query.data && !query.data.detail) {
    const orderData = query.data;
    const sellersMap = new Map();
    let units = 0;
    const productsById: Record<string, any> = {};
    
    const formattedShipments = (orderData.shipments || []).map((shipment: any) => {
      if (shipment.seller) sellersMap.set(shipment.seller.id, shipment.seller);
      for (const item of shipment.items || []) {
        ...
        productsById[item.id] = {
           ...
           description: '',
           stock: 10,
           sku: item.variantLabel || item.variantId
        }
      }
      const normStatus = String(shipment.status || 'placed').toLowerCase();

      const rawEvents = (shipment.events as any[]) || [];
      const events = rawEvents.length > 0
        ? rawEvents.map((ev: any, idx: number) => {
            const code = String(ev.code || ev.status || 'placed').toLowerCase() as any;
            return {
              id: ev.id || `${shipment.id}-evt-${idx}`,
              at: ev.at || new Date().toISOString(),
              code,
              label: ev.label || statusMeta('shipment', code)?.label || code,
              actor: ev.actor || 'admin',
        ...
        : [
            {
              id: `${shipment.id}-evt-placed`,
              at: orderData.placedAt || new Date().toISOString(),
              code: 'placed',
              label: 'Order placed',
              actor: 'shopper'
            }
      ...
      return {
         ...
         canCancel: ['placed', 'confirmed', 'packed', 'PLACED', 'CONFIRMED', 'PACKED'].includes(shipment.status),
    ...
    query.data = {
      detail: {
        order: {
          ...
          shipmentIds: (orderData.shipments || []).map((s: any) => s.id),
          totals: orderData.totals || {
            mrp: orderData.paymentAmount || 0,
            discount: 0,
            coupon: 0,
            shipping: 0,
            codFee: 0,
            total: orderData.paymentAmount || 0
          },
          shipTo: orderData.shipTo || {},
          payment: {
            method: String(orderData.paymentMethod || 'cod').toLowerCase(),
            status: String(orderData.paymentStatus || 'pending').toLowerCase(),
            amount: orderData.paymentAmount,
            txnRef: orderData.paymentTxnRef,
            paidAt: orderData.paymentPaidAt,
            failureReason: orderData.paymentFailed,
            refunds: (orderData as any).refunds || []
          }
        },
  ...
  const advanceShipment = async (shipmentId: string, toStatus: ShipmentStatus) => {
    try {
      const token = getAuthToken('admin') || await ensureAuthToken('admin');
      const res = await fetch(`${API_BASE_URL}/admin/orders/${orderId}/shipments/${shipmentId}/status`, {
        method: 'PUT',
        headers: {
      ...
      toast.success('Status updated', { description: `Shipment moved to ${toStatus.replace(/_/g, ' ')}.` })
    } catch (e: any) {
  ...
  const summary = deriveOrderSummary(detail.shipments.map((entry: any) => entry.shipment))
  const refundable = detail.shipments
    .map((entry: any) => entry.return)
    .filter(
      (request: any): request is ReturnRequest =>
        request !== undefined && request.status !== 'refunded' && request.status !== 'rejected',
    )
  const cancellable = detail.shipments.filter((entry: any) => entry.canCancel)
  ...
  const submitOverride = async () => {
    ...
    const text = note.trim() || `Status updated to ${nextStatus.replace(/_/g, ' ')} by admin`
    try {
      const token = getAuthToken('admin') || await ensureAuthToken('admin');
      const res = await fetch(`${API_BASE_URL}/admin/orders/${orderId}/shipments/${dialog.shipmentId}/status`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
        },
        body: JSON.stringify({ status: nextStatus, note: text })
      });
      if (!res.ok) throw new Error('Failed');
      useSocketStore.getState().bumpOrderTick();
      close()
      ...
      toast.error('That status could not be set', { description: e.message })
    }
  }
      ...
      <PageHeader
        ...
        meta={
          <>
            ...
            <span className="tabular">{formatINR(order.totals?.total ?? 0)}</span>
            <span>
              {order.shipTo?.city ?? 'Unknown'} · {order.shipTo?.pin ?? ''}
            </span>
          </>
        }
        actions={
          ...
          {shipments.map((entry: any) => {
            ...
            const upcoming =
              shipment.status === 'cancelled' || statusIndex < 0
                ? []
                : SHIPMENT_PROGRESS.slice(statusIndex + 1).map((code) => ({
                    code,
                    label: statusMeta('shipment', code).label,
                  }))
              ...
              <SectionCard
                ...
                actions={
                  <div className="flex items-center gap-2">
                    <Button size="sm" variant="outline" onClick={() => deferred(() => openUpdate(shipment.id, shipment.status))}>
                      Change status
                    </Button>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                ...
                <div className="flex flex-col gap-5">
                      <ul className="flex flex-col divide-y divide-border-subtle">
                        {entry.items.map((item: any) => {
                          const productInfo = query.data.productsById[item.id]
                          return (
                            <li key={item.id} className="flex gap-3 py-3 first:pt-0 last:pb-0">
                              ...
                              <div className="flex min-w-0 flex-1 flex-col gap-1">
                                <div className="flex flex-wrap items-center gap-2">
                                  <Link to={`/admin/products/${item.productId}`} className="line-clamp-2 type-body font-medium text-fg hover:underline">
                                    {item.title}
                                  </Link>
                                  <Link
                                    to={`/p/${productInfo?.slug || item.productId}`}
                                    target="_blank"
                                    rel="noreferrer"
                                    className="inline-flex items-center gap-1 type-caption text-primary hover:underline"
                                  >
                                    View on store <ExternalLink className="size-3" aria-hidden />
                                  </Link>
                                </div>
                                <div className="flex flex-wrap items-center gap-2 type-caption text-fg-muted">
                                  {productInfo?.brand ? <span className="font-medium text-fg">{productInfo.brand}</span> : null}
                                  {productInfo?.categoryName ? <span>· {productInfo.categoryName}</span> : null}
                                  <span>· SKU {productInfo?.sku || item.variantId}</span>
                                ...
                                {productInfo?.description ? (
                                  <details className="mt-1 group">
                                    <summary className="cursor-pointer type-caption text-primary group-open:text-fg-muted select-none">
                                      About product details ▾
                                    </summary>
                                    <div className="mt-1.5 rounded-card bg-surface-2 p-2.5 text-fg-muted type-caption flex flex-col gap-1">
                                      <p><strong className="text-fg">Description:</strong> {productInfo.description}</p>
                                      <p><strong className="text-fg">Current inventory:</strong> {formatNumber(productInfo.stock)} units in stock</p>
                                    </div>
                                  </details>
                                ) : null}
                              </div>
                              <p className="shrink-0 type-body text-fg tabular font-medium">{formatINR(item.price * item.qty)}</p>
                            </li>
                          )
                        })}
                      </ul>
                        ...
                        <div className="flex flex-col gap-2">
                          <div className="flex items-center justify-between">
                            ...
                            {shipment.status === 'placed' ? (
                              <Button size="sm" onClick={() => advanceShipment(shipment.id, 'confirmed')}>
                                Confirm
                              </Button>
                            ) : shipment.status === 'confirmed' ? (
                              <Button size="sm" leftIcon={<Package className="size-3.5" aria-hidden />} onClick={() => advanceShipment(shipment.id, 'packed')}>
                                Mark packed
                              </Button>
                            ) : shipment.status === 'packed' ? (
                              <Button size="sm" leftIcon={<Truck className="size-3.5" aria-hidden />} onClick={() => advanceShipment(shipment.id, 'shipped')}>
                                Ship (Delhivery)
                              </Button>
                            ) : shipment.status === 'shipped' ? (
                              <Button size="sm" leftIcon={<Truck className="size-3.5" aria-hidden />} onClick={() => advanceShipment(shipment.id, 'out_for_delivery')}>
                                Out for delivery
                              </Button>
                            ) : shipment.status === 'out_for_delivery' ? (
                              <Button size="sm" leftIcon={<Check className="size-3.5" aria-hidden />} onClick={() => advanceShipment(shipment.id, 'delivered')}>
                                Mark delivered
                              </Button>
                            ) : null}
                          </div>
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <DescriptionList
                        layout="inline"
                        items={[
                          { term: 'Courier', detail: shipment.courier ?? 'Standard Surface' },
                          ...(shipment.awb ? [{ term: 'AWB', detail: shipment.awb, copyValue: shipment.awb }] : []),
                          {
                            term: 'Weight',
                            detail: shipment.package ? `${shipment.package.weightKg} kg` : `${Math.max(0.2, entry.items.length * 0.35).toFixed(2)} kg (estimated)`,
                          },
                          {
                            term: 'Dimensions',
                            detail: shipment.package ? `${shipment.package.dimensionsCm.join(' × ')} cm` : '20 × 15 × 8 cm (standard)',
                          },
          ...
          <SectionCard title="Payment" description="What the shopper paid, and anything sent back.">
            <div className="flex flex-col gap-4">
              <DescriptionList
                columns={2}
                items={[
                  { term: 'Method', detail: (PAYMENT_METHOD_META as any)[order.payment.method]?.label ?? order.payment.method },
              ...
              {(order.payment.refunds || []).length > 0 ? (
                <div className="flex flex-col gap-2">
                  ...
                  <ul className="flex flex-col divide-y divide-border-subtle">
                    {(order.payment.refunds || []).map((refund: any) => (
          ...
          <SectionCard title="Shopper">
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-3">
                <Avatar name={customer?.name ?? order.shipTo?.name ?? 'Chowk shopper'} />
                <div className="flex min-w-0 flex-col">
                  {customer ? (
                    ...
                    <span className="type-label text-fg">{order.shipTo?.name ?? 'Customer'}</span>
                  )}
                  <span className="type-caption text-fg-muted">
                    {customer ? maskEmail(customer.email) : (order.shipTo ? 'Direct shopper' : 'No contact details')}
                  </span>
                </div>
              </div>
              <DescriptionList
                items={[
                  { term: 'Mobile', detail: customer ? maskPhone(customer.phone) : (order.shipTo?.phone ? maskPhone(order.shipTo.phone) : '—') },
                  {
                    term: 'Delivery address',
                    detail: order.shipTo?.name ? `${order.shipTo.name}, ${order.shipTo.line1 || ''}${order.shipTo.line2 ? `, ${order.shipTo.line2}` : ''}, ${order.shipTo.city || ''}, ${order.shipTo.state || ''} ${order.shipTo.pin || ''}` : 'Direct shopper',
                  },
                  ...(order.billing
          ...
          {refundable.length === 0 ? (
            ...
            <fieldset className="flex flex-col gap-2">
              ...
              <RadioGroup
                ...
                options={refundable.map((request: any) => ({
                  value: request.id,
