...
export default function AdminReportsPage() {
  ...
  const seriesByDay = new Map<string, Record<string, number | string>>(
    (data?.series ?? []).map((point: any) => [String(point.day), point]),
  )
  const baseRows: ReportRow[] = (data?.rows ?? []).map((row: any) => {
  ...
  const csvHeaders = (data?.columns ?? []).map((column: any) => column.label)
  const csvRows = () => rows.map((row: any) => (data?.columns ?? []).map((column: any) => row[column.key] ?? ''))
                ...
                {barSpec ? (
                  ...
                  <TrendChart
                    ...
                    series={(timeSpec?.keys ?? []).map((key, index) => ({
                      key,
                      label: data?.columns.find((column: any) => column.key === key)?.label ?? key,
                      slot: ((index % 6) + 1) as 1 | 2 | 3 | 4 | 5 | 6,
                      format: (value) =>
                        data?.columns.find((column: any) => column.key === key)?.format === 'inr'
                          ? formatINR(value)
                          : formatNumber(value),
                    }))}
