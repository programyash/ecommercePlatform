...
export default function CheckoutPaymentPage() {
  ...
  const pay = async () => {
    ...
    try {
      // 1. Ensure authenticated session token
      const token = await ensureAuthToken('customer')
      const payload = {
      ...
      if (response.ok && result.success) {
        ...
        // Synchronize into the reactive store overlay so Admin, Seller, and Shopper views immediately reflect this order
        dbActions.placeOrder({
        ...
        clearCart()
        resetCheckout()
        setPlacedId(backendOrderId)
        return
      }

      if (response.status === 402 || (result.message && result.message.toLowerCase().includes('failed'))) {
    ...
    // 2. Primary / Demo placement via store data layer
    window.setTimeout(() => {
