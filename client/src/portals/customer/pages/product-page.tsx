...
export default function ProductPage() {
  ...
  // 1. Primary data source: full-fidelity local catalogue & store
  ...
  // 2. Secondary fallback for dynamically created backend API products
  const [apiData, setApiData] = useState<any>(null)
  const [apiStatus, setApiStatus] = useState<'idle' | 'loading' | 'success' | 'empty'>('idle')

  useEffect(() => {
    if (demoQuery.status === 'loading' || demoQuery.data) {
      setApiData(null)
      setApiStatus('idle')
      return
    }
    ...
    fetch(`${API_BASE_URL}/products/${slug}`)
      ...
      .then((json) => {
        ...
        const mappedData = {
          detail: {
            product: {
              ...p,
              media: p.variants?.[0]?.images || p.media || [],
              axes: p.axes || [],
              variants: p.variants || [],
            },
            seller: p.seller,
            category: p.category,
            path: p.category ? [{ id: p.categoryId, name: p.category.name, slug: p.category.slug }] : [],
            price: p.variants?.[0]?.price ?? 0,
            mrp: p.variants?.[0]?.mrp ?? 0,
            discount: Math.max(0, (p.variants?.[0]?.mrp ?? 0) - (p.variants?.[0]?.price ?? 0)),
            stock: p.variants?.reduce((sum: number, v: any) => sum + (v.stock || 0), 0) ?? 0,
            stockStatus: (p.variants?.[0]?.stock ?? 0) > 0 ? 'in_stock' : 'out_of_stock',
            offers: [],
            rating: p.rating || { avg: 4.5, count: 12, dist: [8, 3, 1, 0, 0] },
            reviewCount: p.reviews?.length ?? 0,
            unavailable: false,
          },
          reviews: p.reviews || [],
          authors: p.reviews?.reduce((acc: any, rev: any) => ({ ...acc, [rev.customerId]: rev.customer?.name }), {}) || {},
          offers: p.variants ? [{ sellerId: p.sellerId, sellerName: p.seller?.displayName, price: p.variants[0].price, stock: p.variants[0].stock }] : [],
          similar: [],
          bundle: [],
          recent: [],
        }
      ...
      .catch(() => {
        if (active) setApiStatus('empty')
      })

    return () => {
      active = false
    }
  }, [slug, demoQuery.status, demoQuery.data])

  const effectiveData = demoQuery.data || apiData
  const effectiveStatus = demoQuery.data
    ? demoQuery.status
    : demoQuery.status === 'loading'
      ? 'loading'
      : apiStatus === 'idle' || apiStatus === 'loading'
        ? 'loading'
        : apiStatus

  const productId = effectiveData?.detail.product.id
  useEffect(() => {
    if (productId) viewProduct(productId)
  }, [productId, viewProduct])

  if (effectiveStatus === 'loading') return <ProductSkeleton />
    ...
    return (
      <div className="mx-auto flex max-w-shop flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <EmptyState
          ...
          action={<Button onClick={demoQuery.retry}>Retry</Button>}
        />
      </div>
    )
  }
  ...
  return <ProductView key={effectiveData.detail.product.id} data={effectiveData} />
}
