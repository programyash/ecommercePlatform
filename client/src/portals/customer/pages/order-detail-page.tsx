...
import { COURIER_NAME, type OrderView } from '@/data'
...
export default function OrderDetailPage() {
  ...
  const query = useApiQuery<OrderView>(`/orders/${orderId}`, [orderId, orderTick])
  ...
  const rawOrder = query.status === 'empty' ? null : query.data
  const order = rawOrder ? {
    ...rawOrder,
    order: {
      ...rawOrder.order,
      payment: {
        ...rawOrder.order.payment,
        method: String(rawOrder.order.payment?.method || 'cod').toLowerCase() as any,
        status: String(rawOrder.order.payment?.status || 'pending').toLowerCase() as any,
        refunds: rawOrder.order.payment?.refunds || [],
      }
    },
    shipments: (rawOrder.shipments || []).map((s) => ({
      ...s,
      shipment: {
        ...s.shipment,
        ...
        <div className="grid items-start gap-4 xl:grid-cols-12">
          ...
          <div className="flex flex-col gap-4 xl:col-span-4">
            ...
            <SectionCard title="Payment" actions={<StatusBadge domain="payment" status={order.order.payment.status} size="sm" />}>
              <DescriptionList
                items={[
                  { term: 'Method', detail: (PAYMENT_METHOD_META as any)[order.order.payment.method]?.label ?? order.order.payment.method },
