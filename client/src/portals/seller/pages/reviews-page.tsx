...
export default function SellerReviewsPage() {
  ...
  const query = useDemoQuery(
    (view) => getSellerReviews(view, sellerId, { filter, q }),
    [sellerId, filter, q],
  )
  const rows = (query.data ?? []).filter((r) => !stars || r.review.rating === Number(stars))
  ...
  const renderRow = (row: SellerReviewRow) => {
    const { review, product, customerName } = row
    return (
    <ReviewCard
      key={review.id}
      review={review}
      ...
      actions={
        replyTo === review.id ? (
          <ReplyForm reviewId={review.id} onDone={() => setReplyTo(null)} onCancel={() => setReplyTo(null)} />
        ) : (
          <>
            {product ? (
              <Link
            ...
            <span className="ml-auto flex flex-wrap items-center gap-2">
              {review.status === 'flagged' ? <StatusBadge domain="review" status="flagged" size="sm" withTooltip /> : null}
              {review.sellerReply ? null : (
                <Button size="sm" variant="outline" leftIcon={<Reply aria-hidden />} onClick={() => setReplyTo(review.id)}>
                  Reply
                </Button>
              )}
              {review.status === 'flagged' ? null : (
                <Button size="sm" variant="ghost" leftIcon={<Flag aria-hidden />} onClick={() => setReportId(review.id)}>
                  Report
                </Button>
              )}
            </span>
