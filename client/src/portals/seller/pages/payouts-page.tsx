...
export default function SellerPayoutsPage() {
  ...
  const query = useDemoQuery((view) => getSellerPayouts(view, sellerId), [sellerId])
  
  // A forced empty state (?demo=empty) still returns real data; the screen must honour it.
  const allRows = query.status === 'empty' ? [] : (query.data ?? [])
  const rows = status === 'all' ? allRows : allRows.filter((payout) => payout.status === status)

  const paid = allRows.filter((payout) => payout.status === 'paid')
  const onHold = allRows.filter((payout) => payout.status === 'on_hold')
    ...
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <SectionCard title="Paid so far">
          <p className="type-kpi text-fg">{formatINR(paid.reduce((sum: number, payout: any) => sum + payout.net, 0))}</p>
          <p className="type-body text-fg-muted">
        ...
        <SectionCard title="On hold">
          <p className="type-kpi text-fg">{formatINR(onHold.reduce((sum: number, payout: any) => sum + payout.net, 0))}</p>
          <p className="type-body text-fg-muted">
            {onHold.length === 0
              ? 'Nothing held back.'
