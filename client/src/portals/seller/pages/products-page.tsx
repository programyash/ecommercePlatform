...
export default function SellerProductsPage() {
  ...
  const soldByProduct = useDb((view) => {
  ...
  const csvHeaders = ['Title', 'Brand', 'Status', 'Price', 'Sold', 'Quantity Left', 'Variants', 'Updated'] as const
  ...
  const columns: Column<Product>[] = [
    ...
    },
    {
      ...
      align: 'end',
      mobile: 'meta',
      sortValue: (product) => soldByProduct.get(product.id) ?? 0,
      cell: (product) => {
        const sold = soldByProduct.get(product.id) ?? 0
        return <span className="type-body text-fg tabular">{formatNumber(sold)}</span>
      },
    },
    {
      id: 'stock',
      ...
      cell: (product) => {
        ...
        return (
          <span className="inline-flex items-center justify-end gap-1.5">
            <span className="text-fg font-medium tabular">{formatNumber(stock)}</span>
