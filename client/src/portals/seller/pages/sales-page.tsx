...
export default function SellerSalesPage() {
  ...
  const dayRows = () => (data?.series ?? []).map((point: any) => [formatDate(`${String(point.day)}T00:00:00+05:30`), Number(point.gmv), Number(point.orders), Number(point.units)])
  ...
  return (
    <>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        >
          <BarBreakdownChart
            data={(data?.status ?? []).map((row: any) => ({ status: statusMeta('shipment', row.status).label, count: row.count }))}
            categoryKey="status"
        ...
        >
          <DonutChart
            data={(data?.categories ?? []).map((row: any) => ({ label: row.name, value: row.gmv }))}
