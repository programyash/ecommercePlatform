import { Ban, Check, Copy, Ellipsis, Package, Printer, RotateCcw, TriangleAlert, Truck } from 'lucide-react'
...
function ItemLine({ item, sku, intraState, productSlug }: { item: OrderItem; sku: string; intraState: boolean; productSlug?: string }) {
    ...
    <li className="flex flex-wrap items-start gap-3 px-4 py-4 sm:px-5">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        <div className="flex flex-wrap items-center gap-2">
          <Link to={`/seller/products/${item.productId}/edit`} className="type-label font-medium text-fg hover:underline">
            {item.title}
          </Link>
          <Link
            to={`/p/${productSlug || item.productId}`}
            target="_blank"
            rel="noreferrer"
...
export default function SellerOrderDetailPage() {
  ...
  const query = useApiQuery<SellerShipmentRow[]>('/sellers/shipments', [orderTick])
  ...
  const rawRow = (query.data ?? []).find(r => r.shipment.id === shipmentId)
  const row: any = rawRow ? {
    ...rawRow,
    shipment: {
      ...rawRow.shipment,
      status: String(rawRow.shipment.status || 'placed').toLowerCase() as any,
    }
  } : undefined
        ...
        <EmptyState
          ...
          description={
            query.status === 'error'
              ...
              : 'It may belong to another seller or the link may be old.'
          }
          action={
            query.status === 'error' ? (
  ...
  const detail: any = {
     ...row,
     seller: undefined, // Usually seller sees their own orders
     earnings: { lines: [], gross: 0, net: row.amount * 0.9 }, // Mock earnings
     couponFundedByPlatform: false,
     payout: undefined
  };

  const packDefault = { weightKg: 0.5, dimensionsCm: [20, 15, 8] as [number, number, number] };
  const skus: Record<string, string> = {};
  const slugs: Record<string, string> = {};
  for (const item of row.items) {
     skus[item.id] = item.variantLabel || item.variantId;
  }
  ...
  const updateStatus = async (status: string, successMsg: string, descMsg: string) => {
    try {
      const res = await fetch(`/api/v1/sellers/shipments/${shipment.id}/status`, {
      ...
      toast.success(successMsg, { description: descMsg })
    } catch (e: any) {
      toast.error('Action failed', { description: e.message })
    }
  }

  const confirm = () => updateStatus('CONFIRMED', 'Order confirmed', `Pack it by ${formatDayShort(shipment.slaDueAt)}.`)
  const handOver = () => updateStatus('READY_FOR_PICKUP', `Handed over to ${COURIER_NAME}`, 'The AWB is now on this order and the shopper can track it.')
  const markOutForDelivery = () => updateStatus('OUT_FOR_DELIVERY', 'Shipment out for delivery', 'The parcel is out for delivery with the courier.')
  const markDelivered = () => updateStatus('DELIVERED', 'Delivered successfully', 'The parcel has been marked as delivered.')
  ...
  const nextAction =
    shipment.status === 'PLACED' ? (
      ...
      <Button leftIcon={<Truck aria-hidden />} onClick={markOutForDelivery}>
        Mark out for delivery
      </Button>
    ) : shipment.status === 'OUT_FOR_DELIVERY' ? (
      <Button leftIcon={<Check aria-hidden />} onClick={markDelivered}>
        Mark delivered
      </Button>
    ) : shipment.status === 'DELIVERED' ? (
      <Badge tone="success" size="md">Delivered</Badge>
    ) : null
          ...
          <>
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                ...
                {shipment.status === 'shipped' ? (
                  <DropdownMenuItem icon={<Truck aria-hidden />} onSelect={markOutForDelivery}>
                    Mark out for delivery
                  </DropdownMenuItem>
                ) : null}
                {shipment.status === 'out_for_delivery' ? (
                  <DropdownMenuItem icon={<Check aria-hidden />} onSelect={markDelivered}>
                    Mark delivered
                  </DropdownMenuItem>
                ) : null}
                {shipment.status !== 'delivered' && shipment.status !== 'cancelled' ? (
                  <DropdownMenuItem
                    ...
                    onSelect={() => {
                      const res = dbActions.advanceCourier(shipment.id)
                      if (res.ok) toast.success('Tracking advanced to next step')
                      else toast.error('Could not advance tracking', { description: res.error })
                    }}
                  >
                    Advance tracking step
                  </DropdownMenuItem>
                ) : null}
            ...
            <ul className="divide-y divide-border-subtle">
              {items.map((item: any) => (
                <ItemLine
                  key={item.id}
                  item={item}
                  sku={skus[item.id] ?? item.variantId}
                  intraState={intraState}
                  productSlug={slugs[item.id]}
                />
              ))}
            </ul>
        ...
        rows={[
          {
            ...
            items: items.map((item: any) => `${item.title} × ${item.qty}`).join(', '),
