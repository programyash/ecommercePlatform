...
import { ensureAuthToken } from '@/lib/auth-token'
...
export default function ProductEditorPage() {
  ...
  // ── Saving ──────────────────────────────────────────────────────────────
  const buildInput = (submit: boolean, publishDirectly = false) => {
    const current = form.getValues()
    const usedAxes = axes.filter((spec) => spec.values.length > 0).map((spec) => spec.axis)
    const uploadedMedia: MediaRef[] = images
      .filter((img) => !img.error && Boolean(img.url))
      .map((img) => img.url as unknown as MediaRef)
  ...
  const persist = async (submit: boolean, message: { title: string; description: string }, publishDirectly = false) => {
    const input = buildInput(submit, publishDirectly)
    const result = dbActions.upsertProduct(input)
    if (!result.ok) {
    ...
    try {
      const token = await ensureAuthToken('seller')
      if (token) {
        const payload = {
          id: result.productId,
          title: input.title,
          ...
          variants: input.variants.map((v, index) => ({
            id: (v as any).id || `${result.productId}_v${index + 1}`,
            sku: v.sku,
            price: v.price,
            mrp: v.mrp,
            stock: v.stock,
        ...
        const endpoint = productId ? `${API_BASE_URL}/sellers/products/${productId}` : `${API_BASE_URL}/sellers/products`
        ...
        if (resData?.data?.id && resData.data.id !== result.productId) {
          // If backend generated a different ID, ensure client overlay reflects it
          dbActions.upsertProduct({
            ...input,
            id: resData.data.id,
            publishDirectly,
          })
    ...
    if (publishDirectly) {
      void navigate('/seller/products?tab=live')
    } else if (submit) {
      void navigate('/seller/products?tab=pending')
    } else if (!productId) {
      void navigate(`/seller/products/${result.productId}/edit`, { replace: true })
    }
  }
  ...
  const onSubmit = () => {
    ...
    persist(true, {
      ...
      description: 'Your product is now live on Chowk and ready for shoppers to purchase.',
    }, true)
  }
            ...
            {readOnly ? (
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  persist(true, {
                    title: 'Listing published',
                    description: 'Your product is now live on Chowk and ready for purchase.',
                  }, true)
                }}
              >
                Publish Live
              ...
              >
                ...
                {section.id === 'variants' ? (
                  <div className="flex flex-col gap-4">
                    ...
                    {axes.length === 0 ? (
                      <div className="rounded-card border border-border bg-surface-2 p-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                          <p className="type-label text-fg">Available Quantity / Stock</p>
                          <p className="type-caption text-fg-muted">Specify how many units of this product you have ready to fulfill.</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <Input
                            type="number"
                            min={0}
                            disabled={readOnly}
                            value={String(rowFor({}).stock)}
                            onChange={(e) => {
                              const val = Math.max(0, parseInt(e.target.value) || 0)
                              setRowData({ ...rowData, [keyOf({})]: { ...rowFor({}), stock: val } })
                            }}
                            placeholder="Units"
                            wrapperClassName="w-28 tabular"
                          />
                          <span className="type-caption text-fg-muted">units</span>
                        </div>
                      </div>
                    ) : null}
                      ...
                      <table className="w-full border-collapse text-left">
                        ...
                        <thead>
                          <tr className="border-b border-border">
                            ...
                            <th scope="col" className="py-2 pr-3 text-right type-caption font-semibold whitespace-nowrap text-fg-muted">Quantity (Stock)</th>
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    {axes.length === 0 ? (
                      <div className="flex flex-col gap-1.5 sm:w-1/2">
                        <Label htmlFor="pricing-stock">Quantity in stock</Label>
                        <Input
                          id="pricing-stock"
                          type="number"
                          min={0}
                          ...
                          placeholder="e.g. 50"
                          wrapperClassName="tabular"
                        />
                        <FieldHint>How many units you have ready to ship.</FieldHint>
                      </div>
                    ) : null}
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  persist(true, {
                    ...
                    description: 'Your listing is in the moderation queue for catalogue approval.',
                  }, false)
                }}
              >
                Submit for review
