import { Boxes, Ellipsis, Pencil, Plus, TriangleAlert, Upload } from 'lucide-react'
...
export default function SellerInventoryPage() {
  ...
  const inventoryTick = useSocketStore(s => s.inventoryTick)
  const query = useApiQuery<InventoryRow[]>('/sellers/products', [sellerId, inventoryTick])

  let rows = query.data ?? []
  if (q || lowOnly) {
     rows = rows.filter(row => {
        let pass = true;
        if (lowOnly) pass = pass && row.status !== 'in_stock';
        if (q) pass = pass && (row.product.title.toLowerCase().includes(q.toLowerCase()) || row.variant.sku.toLowerCase().includes(q.toLowerCase()));
        return pass;
     });
  }
