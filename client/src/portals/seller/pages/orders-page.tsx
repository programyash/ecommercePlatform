...
export default function SellerOrdersPage() {
  ...
  const query = useApiQuery<SellerShipmentRow[]>('/sellers/shipments', [sellerId, orderTick])

  // Process rows through filters
  const allRows = (query.data ?? []).map((row) => ({
    ...row,
    shipment: {
      ...row.shipment,
      status: String(row.shipment.status || 'placed').toLowerCase() as any,
    }
  }))
  let rows = allRows;
  ...
  if (tabConfig) {
    rows = rows.filter(row => {
      if (tabConfig.returnsOnly) return Boolean(row.return);
      return tabConfig.statuses.includes(row.shipment.status);
    });
  }

  // Payment, due, placed filters
  if (payment !== 'all') {
    rows = rows.filter(row => payment === 'prepaid' ? row.prepaid : !row.prepaid);
  }
  if (due !== 'all') {
    rows = rows.filter(row => due === 'overdue' ? row.overdue : (toDayKey(row.dueAt) === toDayKey(DEMO_NOW) && row.action));
  }
  if (placed !== 'all') {
  ...
  if (q) {
    ...
    rows = rows.filter(row => 
      row.shipment.id.toLowerCase().includes(queryLower) ||
      row.order.id.toLowerCase().includes(queryLower) ||
      row.buyer.toLowerCase().includes(queryLower) ||
      row.items.some((item) => item.title.toLowerCase().includes(queryLower))
    );
  }
  ...
  for (const row of allRows) {
     defaults[row.shipment.id] = { weightKg: 0.5, dimensionsCm: [20, 15, 8] }
     skus[row.shipment.id] = row.items[0]?.variantLabel ?? ''
  }

  const counts = {
    new: allRows.filter(r => r.shipment.status === 'placed').length,
    to_pack: allRows.filter(r => r.shipment.status === 'confirmed' || r.shipment.status === 'processing').length,
    ready_to_ship: allRows.filter(r => r.shipment.status === 'packed' || r.shipment.status === 'ready_for_pickup').length,
    in_transit: allRows.filter(r => ['picked_up', 'in_transit', 'shipped', 'out_for_delivery'].includes(r.shipment.status)).length,
    delivered: allRows.filter(r => r.shipment.status === 'delivered').length,
    cancelled: allRows.filter(r => r.shipment.status === 'cancelled').length,
    returns: allRows.filter(r => Boolean(r.return)).length,
  }
  ...
  const confirmOne = async (row: SellerShipmentRow) => {
    try {
      const res = await fetch(`/api/v1/sellers/shipments/${row.shipment.id}/status`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${getAuthToken('seller') || localStorage.getItem('token')}` },
        body: JSON.stringify({ status: 'CONFIRMED' })
      });
      if (!res.ok) throw new Error('Failed');
      useSocketStore.getState().bumpOrderTick();
      toast.success('Order confirmed', { description: row.shipment.id })
    } catch (e: any) {
      toast.error('Could not confirm order', { description: e.message })
    }
  }

  const handOverOne = async (row: SellerShipmentRow) => {
    try {
      const res = await fetch(`/api/v1/sellers/shipments/${row.shipment.id}/status`, {
        method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${getAuthToken('seller') || localStorage.getItem('token')}` },
      ...
      if (!res.ok) throw new Error('Failed');
      useSocketStore.getState().bumpOrderTick();
      toast.success('Handed over to courier', { description: row.shipment.id })
    } catch (e: any) {
      toast.error('Could not hand over order', { description: e.message })
    }
  }

  const bulkConfirm = async (ids: string[], clear: () => void) => {
    let count = 0
    for (const id of ids) {
      try {
        const res = await fetch(`/api/v1/sellers/shipments/${id}/status`, {
          method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${getAuthToken('seller') || localStorage.getItem('token')}` },
          body: JSON.stringify({ status: 'CONFIRMED' })
        });
        if (res.ok) count++
    ...
    useSocketStore.getState().bumpOrderTick();
    clear()
    toast.success(`${count} ${count === 1 ? 'order' : 'orders'} confirmed`)
  }

  const bulkHandOver = async (ids: string[], clear: () => void) => {
    let count = 0
    for (const id of ids) {
      try {
        const res = await fetch(`/api/v1/sellers/shipments/${id}/status`, {
    ...
    useSocketStore.getState().bumpOrderTick();
    clear()
    toast.success(`${count} ${count === 1 ? 'order' : 'orders'} handed over to courier`)
  }

  const bulkPack = async (ids: string[], clear: () => void) => {
    const targets = rows.filter((row) => ids.includes(row.shipment.id) && (row.shipment.status === 'confirmed' || (row.shipment.status as any) === 'CONFIRMED'))
    ...
    for (const row of targets) {
      try {
        await fetch(`/api/v1/sellers/shipments/${row.shipment.id}/status`, {
          method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${getAuthToken('seller') || localStorage.getItem('token')}` },
        ...
        bulkActions={(ids, clear) => {
          ...
          return (
            <>
              ...
              <Button size="sm" variant="outline" leftIcon={<Truck aria-hidden />} onClick={() => bulkHandOver(ids, clear)}>
                Ready for pickup
              </Button>
