    ...
    (set) => ({
      ...
      setQty: (lineId, qty, stock) =>
        set((state) => {
          ...
          if (line) {
            apiCall(`/customers/cart/${line.variantId}`, 'PUT', { qty: clamp(qty, stock) });
          }
          return {
      ...
      remove: (lineId) => set((state) => {
        const line = state.lines.find(l => l.lineId === lineId);
        if (line) apiCall(`/customers/cart/${line.variantId}`, 'DELETE');
      ...
      saveForLater: (lineId) =>
        set((state) => {
          ...
          apiCall(`/customers/cart/${line.variantId}`, 'DELETE');
          apiCall(`/customers/wishlist`, 'POST', {
      ...
      moveToCart: (lineId) =>
        set((state) => {
          ...
          if (!line) return state
          apiCall(`/customers/wishlist/${line.variantId}`, 'DELETE');
          apiCall(`/customers/cart`, 'POST', {
            productId: line.productId,
            variantId: line.variantId,
            sellerId: line.sellerId,
          ...
          return {
            ...
            lines: state.lines.some((entry) => entry.lineId === lineId)
              ? state.lines.map((entry) => (entry.lineId === lineId ? { ...entry, qty: entry.qty + line.qty } : entry))
              : [line, ...state.lines],
          }
        }),
      ...
      clear: () => {
        apiCall(`/customers/cart`, 'DELETE');
        set({ lines: [], saved: [], couponCode: null, buyNow: null, checkout: EMPTY_CHECKOUT })
      },
