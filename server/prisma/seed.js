const { PrismaClient, Role, CustomerStatus, SellerStatus, ProductStatus } = require("@prisma/client");
...
async function main() {
  ...
  // Also create customer1@example.com
  await prisma.user.create({
    data: {
  ...
  const parents = seedData.categories.filter((c) => !c.parentId);
  const children = seedData.categories.filter((c) => Boolean(c.parentId));
  ...
  // Also create seller1@example.com for test credentials
  await prisma.user.create({
    data: {
